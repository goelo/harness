...
| `.harness/scripts/verify.py` | 提交前质量检查 CLI | ✓ |
...
进入 harness 实现前必须先用 `grill-me` 确认需求边界、验收条件和关键设计选择。即使 `design.md` 看起来已经完整,也先完成这一轮确认,再创建 harness task。
...
| # | 时机 | 你做什么 |
|---|-----|---------|
| 1 | grill-me 完成后 | design.md 和追问答案是否可以作为实现依据? |
...
```
1.  确认项目根有 design.md
2.  Mandatory grill-me: task.py create 之前完成需求确认
3.  task.py create
4.  curate 3 个 manifest (context.{architect,developer,tester}.jsonl)
5.  dispatch architect → 出 info.md + scope.json
6.  验证 info.md 包含可测契约
...
11. 主会话运行 verify.py all 后 commit
...
- task.py CLI 的状态机
- verify.py 提交前 lint/type/test/coverage/scope 检查
...
- **4 个 checkpoint 替代实时纠错。** 让你做决策,AI 做劳动。
