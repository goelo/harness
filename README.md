...
| 变量 | 默认 | 说明 |
|------|-----|------|
| `HARNESS_REPO` | `https://github.com/goelo/harness.git` | 自定义 fork 时改这里 |
| `HARNESS_BRANCH` | `main` | 装其他分支时改 |
...
- `python3 ≥ 3.10`
- `git`
- 可访问 `https://github.com/goelo/harness.git`
...
跨项目共享:
- **RTK**(上下文优化)— 全局装,一次到位
- **Caveman**(紧凑输出模式)— 公开版本保留跳过参数,自动安装入口由使用环境自行配置
...
GitHub: `https://github.com/goelo/harness`

发现问题或想加 feature,可以直接在仓里开 issue 或 PR。
