...
Bad (typo-prone): `| "sample123" | length 20 + lowercase 10 + digit 15 | 35 |` ← 35 ≠ 20+10+15=45
...
def install_caveman(skip: bool, dry_run: bool) -> str:
    """Best-effort Caveman auto-install via a configured curl|bash script.

    Status values: skipped / already_installed / would_install / would_skip /
    installed / install_failed / needs_curl / needs_bash / unavailable.
    """
    if skip:
        return "skipped"
    if _caveman_present():
        return "already_installed"
    if not CAVEMAN_INSTALL_URL:
        return "unavailable"
...
def report_caveman_status(status: str) -> None:
    ...
    messages = {
        ...
        "unavailable":       "Caveman auto-install is unavailable in this public build.",
        ...
        "needs_curl":        "Caveman not installed and curl not found.\n"
                             "    Install curl first, then run the configured Caveman installer manually.",
        "needs_bash":        "Caveman not installed and bash not found.\n"
                             "    Install bash, then run the configured Caveman installer manually.",
    }
