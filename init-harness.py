...
name: harness-implement
description: |
  在 DeepSeek TUI 中，当协作者要求依据 design.md、spec.md、requirements.md
  等需求文档实现功能，且项目已经初始化 harness 时使用。
  触发语: "按照 design.md 开发", "implement design.md", "按照详细设计文档开发",
  "用 harness 实现 xxx.md", "走 harness 流程"，以及任何把 Markdown 需求文档作为实现依据的表达。
---
...
DeepSeek TUI 不会自动收到 Claude Code hook 事件。需要把 hook 原本承担的
动作作为显式步骤执行：

1. 使用 `.harness/scripts/task.py` 管理 task 状态。
2. 在打开或重新介入 sub-agent 之前，使用 `.harness/scripts/context.py`
   生成角色上下文。
3. 使用 `agent_open`、`agent_eval` 和 `agent_close`，替代 Claude Team APIs。
...
需求文档是产品需求来源，保持原文完整。architect 负责把需求转换成
`info.md` 中可测试的技术契约。
...
| --- | --- | --- |
| SessionStart / UserPromptSubmit | automatic hook | 显式运行 `task.py` 并检查 task package |
| PreToolUse context injection | automatic hook | 运行 `context.py <role> --task <task-dir>`，并把输出放入 prompt |
| Team-based dispatch | Claude Team APIs | `agent_open`, `agent_eval`, `agent_close` |
| Persistent role teammate | named team agent | 命名的 DeepSeek child session |
...
| 5 | 使用 `context.py` 生成 architect 上下文，然后打开 architect child session 写入 `info.md` 和 `scope.json`。 |
| 6 | 检查 `info.md` 是否包含可测试契约和 slice 计划。 |
| 7 | 运行 `python3 .harness/scripts/task.py start <task-dir>`。 |
| 8 | 完成执行模式确认，询问是否启用 3-agent mode。 |
| 9 | 如果确认启用，保持 architect/tester/developer child sessions 打开；否则只在阶段需要时打开对应角色 session。 |
...
把命令输出作为 child session prompt。该步骤替代 Claude Code 自动
...
继续下一阶段前，使用 `agent_eval` 并设置 `block: true` 等待 child session
完成。已完成角色 session 使用 `agent_close` 关闭。

只有在协作者确认 3-agent mode 后，才同时打开三个角色 session。没有该确认时，
只为当前阶段打开对应 child session，完成后关闭；只有下一阶段马上复用时才保持空闲。
...
4. 每个 DeepSeek child-session prompt 都必须包含 `context.py` 输出。
5. 每个 `agent_open` prompt 都必须写明角色写入边界。
...
该 skill 同时供 Claude Code、DeepSeek TUI 和 Codex agents 使用。
