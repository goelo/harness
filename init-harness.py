...
LEGACY_MANAGED_SKILL_MARKERS = {
    ...
    "project-doc-scanner": (
        "先检测，再确认，再扫描",
        ".harness/scripts/project.py docs status",
    ),
}
...
def create_skill_files(
    skills_dir: Path,
    harness_implement: str | None = None,
    *,
    include_project_doc_scanner: bool = True,
) -> None:
    ...
    if include_project_doc_scanner:
        project_doc_scanner_dir = skills_dir / "project-doc-scanner"
        project_doc_scanner_dir.mkdir(parents=True, exist_ok=True)
        _write_managed_skill(
            project_doc_scanner_dir / "SKILL.md",
            SKILL_PROJECT_DOC_SCANNER,
            "project-doc-scanner",
        )
...
def create_codex_skills() -> None:
    ...
    create_skill_files(
        ...
        include_project_doc_scanner=False,
    )
...
PROJECT_PY_STUB = """\
#!/usr/bin/env python3
\"\"\"project.py — placeholder. Real implementation in harness_scripts/.\"\"\"
import sys
print("project.py: real implementation not deployed", file=sys.stderr)
sys.exit(1)
"""
