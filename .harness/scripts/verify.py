...
def run_red_green(root: Path, args: argparse.Namespace, *, mode: str) -> int:
    ...
    payload = {
        ...
        "stdout": result.stdout[-4000:],
        "stderr": result.stderr[-4000:],
        "contractCoverage": parse_contract_coverage(args.contract_coverage),
        "uncoveredContracts": args.uncovered_contract or [],
        "finishedAt": utc_now(),
...
def parse_contract_coverage(values: list[str] | None) -> dict[str, list[str]]:
    coverage: dict[str, list[str]] = {}
    for raw in values or []:
        if "=" not in raw:
            print(f"Error: invalid contract coverage mapping: {raw}", file=sys.stderr)
            sys.exit(2)
        contract_id, tests = raw.split("=", 1)
        contract_id = contract_id.strip()
        target_tests = [item.strip() for item in tests.split(",") if item.strip()]
        if not contract_id or not target_tests:
            print(f"Error: invalid contract coverage mapping: {raw}", file=sys.stderr)
            sys.exit(2)
        coverage.setdefault(contract_id, [])
        for target_test in target_tests:
            if target_test not in coverage[contract_id]:
                coverage[contract_id].append(target_test)
    return coverage
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--contract-coverage", action="append", help="Business contract coverage mapping, for example BC-001=TestA,TestB")
    parser.add_argument("--uncovered-contract", action="append", help="Business contract id without a current test mapping")
